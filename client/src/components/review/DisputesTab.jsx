...
export default function DisputesTab() {
  ...
  if (loading) return <div className="py-8 text-gray-400 text-sm">Loading disputes…</div>;
  if (error) return <div className="py-4 text-red-600 text-sm">{error}</div>;
          ...
          <div key={gi} className="bg-white rounded-xl border border-gray-200 overflow-hidden">
            ...
            <div className="divide-y divide-gray-100">
              {g.items.map(d => {
                ...
                const isSel = selected === d.score_id;
                return (
                  <button
                    key={d.score_id}
                    onClick={() => openDispute(d)}
                    className={`w-full text-left px-4 py-3 hover:bg-gray-50 transition-colors ${
                      isSel ? 'bg-gray-50 border-l-4 border-gray-900' : ''
                    }`}
                  >
                    ...
                    <div className="flex items-center gap-3 mt-1">
                      ...
                      <span className="text-xs font-medium text-gray-500 ml-auto">Δ {diff.toFixed(1)}</span>
                    </div>
                  </button>
                );
              })}
          ...
          <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
            ...
            <div className="p-6 space-y-6">
              ...
              <form onSubmit={handleResolve} className="space-y-4 border-t border-gray-200 pt-5">
                ...
                {finalScore !== '' && (() => {
                  const val = parseFloat(finalScore);
                  const selfVal = selectedDispute.self_score;
                  const mgrVal = selectedDispute.manager_score;
                  const selfDiff = selfVal !== null ? Math.abs(val - selfVal).toFixed(1) : null;
                  const mgrDiff = mgrVal !== null ? Math.abs(val - mgrVal).toFixed(1) : null;
                  return (
                    <div className="bg-gray-50 border border-gray-200 rounded-lg p-3">
                      <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">Resolution Preview</p>
                      <div className="flex items-center gap-3 text-sm">
                        <div className="text-center">
                          <div className="text-xs text-gray-400 mb-0.5">Self</div>
                          <div className="font-semibold text-gray-700">{selfVal ?? '—'}</div>
                        </div>
                        <div className="text-gray-300">|</div>
                        <div className="text-center">
                          <div className="text-xs text-gray-400 mb-0.5">Manager</div>
                          <div className="font-semibold text-gray-500">{mgrVal ?? '—'}</div>
                        </div>
                        <div className="text-gray-300">→</div>
                        <div className="text-center">
                        ...
                        {selfDiff && mgrDiff && (
                          <div className="ml-auto text-xs text-gray-400">
                            <div>Δ self: {selfDiff}</div>
                            <div>Δ mgr: {mgrDiff}</div>
                          </div>
                        )}
                      </div>
                    </div>
